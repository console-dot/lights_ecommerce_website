import light1 from "./Light1.jpg";
import light1Bg from "./Light1Bg.jpg";
import light2 from "./Light2.jpg";
import light2Bg from "./Light2bg.jpg";
import BennerImage1 from "./Main-banner-1-1920x930.jpg";
import BennerImage2 from "./Main-banner-2-1920x930.jpg";
import BennerImage3 from "./Main-banner-3-1920x930.jpg";
import parallax from "./parallax.jpg";
import user1 from "./user1.jpg";
import user2 from "./user2.jpg";
import user3 from "./user3.jpg";
import logo from "./logo.png";
import menuImg from "./menu-01.jpg";
import glowPlate1 from "./Glowplate1.jpg";
import glowPlate2 from "./Glowplate2.jpg";
import glowPlate3 from "./Glowplate3.jpg";
import glowPlate4 from "./Glowplate4.jpg";
import glowPlate5 from "./Glowplate5.jpg";
import categoryCardIamge from "./cms-banner-1.jpg";
import categoryCardIamge1 from "./cms-banner-2.jpg";
import newLetterImage from "./newsletter-img.jpg";
import lightHouse from "./lightHouse.jpg";
import lightHouse2 from "./lightHouse2.jpg";
import dignoalLines from "./dignoalLine.png";
import decorateLight1 from "./decorateLight1.jpg";
import decorateLight2 from "./decorateLight2.jpg";
import ProductDetailImage from "./ProductDetailsImage.jpg";
import blog1 from "./blog1-870x573.jpg";
import blog2 from "./blog2-870x573.jpg";
import blog3 from "./blog4-870x573.jpg";
import wallLightbg from "./walllightbg.png";
import wallLight1bg from "./walllight1bg.jpg";
import wallLight from "./walllight.png";
import wallLight1 from "./walllight1.png";
import login from "./led-823383_640.jpg";
import login1 from "./How-to-choose-the-perfect-decor-lights-for-your-room-FB-1200x700-compressed.jpg";
import indoorLightbg from "./indoorlight.png";
import indoorLight from "./indoorlightbg.png";
import multiLight from "./multiply.png";
import multiLightbg from "./multiplybg.jpg";
import restaurantLight from "./restaurant.png";
import restaurantLightbg from "./restrunentbg.jpg";
import restaurantLight1 from "./restaurant1.png";
import restaurantLight1bg from "./restrunent1bg.jpg";
import table from "./tabel.png";
import tablebg from "./tabelbg.jpg";
import table1 from "./tabellamp1.png";
import table1bg from "./tabellamp1bg.jpg";
import table2 from "./tabellamp2.png";
import table2bg from "./tabellamp2bg.jpg";
import table3 from "./tabellamp3.png";
import table3bg from "./tabellamp3bg.jpg";
import table4 from "./tabellamp4.png";
import table4bg from "./tabellamp4bg.jpg";
import table5 from "./tabellamp5.png";
import table5bg from "./tabellamp5bg.jpg";
import table6bg from "./tabellamp6.jpg";
import table6 from "./tabellamp6bg.png";
import commercialLight from "./Commercial Light.png"
import commercialLightbg from "./Commercial Lightbg.jpg"
import order from "./order.png"

export {
  order,
  light1Bg,
  light2,
  light1,
  light2Bg,
  BennerImage1,
  BennerImage2,
  BennerImage3,
  parallax,
  user1,
  user2,
  user3,
  categoryCardIamge1,
  categoryCardIamge,
  logo,
  menuImg,
  glowPlate1,
  glowPlate2,
  glowPlate3,
  glowPlate4,
  glowPlate5,
  newLetterImage,
  lightHouse,
  lightHouse2,
  dignoalLines,
  decorateLight1,
  decorateLight2,
  ProductDetailImage,
  blog1,
  blog2,
  blog3,
  wallLight,
  wallLight1,
  wallLight1bg,
  wallLightbg,
  login,
  login1,
  indoorLight,
  indoorLightbg,
  multiLight,
  multiLightbg,
  restaurantLight,
  restaurantLightbg,
  restaurantLight1,
  restaurantLight1bg,
  table,
  table1,
  table1bg,
  tablebg,
  table2,
  table2bg,
  table3,
  table3bg,
  table4,
  table4bg,
  table5,
  table5bg,
  table6,
  table6bg,commercialLight,commercialLightbg
};
